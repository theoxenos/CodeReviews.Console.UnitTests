...
public class SessionLogRepository
{
    ...
    public void DeleteSessionLog(SessionLog log)
    {
        ...
        if (rows == 0) throw new InvalidOperationException("Session log with specified ID not found.");
    }
    ...
    public void UpdateSessionLog(SessionLog sessionLog)
    {
        ...
        using var connection = db.GetConnection();
        connection.Execute(
            "UPDATE Logs SET StartTime = @StartTime, EndTime = @EndTime, Duration = @Duration WHERE Id = @Id",
            sessionLog);
    }
